...
const itemCounts = Total_data.reduce((counts, order) => {
    const itemType = order.item_type;
    counts[itemType] = (counts[itemType] || 0) + 1;
    return counts;
  }, {});
  
  // Create a new list with key-value pairs (name and count)
const data = Object.entries(itemCounts).map(([name, count]) => ({
name,
count,
}));
